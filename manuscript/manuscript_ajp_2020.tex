...
\begin{abstract}
  ...
  Our results show a variety of systems that
  synchronize 
  %and detect synchronization using
  %measurements of
  particle velocity as a function of applied driving force,
  and characterize these behaviors
  with force-velocity curves and phase diagrams.
...
Experiments with colloid particles,
such as plastic beads suspended in
highly de-ionized water \cite{},
are useful model systems
to understand complex behaviors.
%silica spheres in organic solvents
%magnetic spheres?
%
...
of individual particles during mode-locking,
...
due to their 
relatively large size of microns,
making them relatively simple 
to control and image.
Additionally colloids can mimic different
systems since 
...
Mode-locking 
in colloids has been achieved in systems interacting via magnetic dipoles 
or electric forces 
...
\label{sec:MD}
In this section we describe the dynamics of the particles
using the model forces
and the motion using an modified form of Newton's second law
suitable for microscopic particles in a suspending fluid.
...
The confining force %arises from a potential function 
...
% 
The periodic landscape is modulated in the y-direction as
%% %that used in Sec.~\ref{sec:one}
% where 
 \begin{equation}
   U_{washboard}(y) = U_0 \cos{(N_p \pi y / L)}
     \label{eq:ysubstrate}
\end{equation}
...
of $R \approx 10^{-6}$, indicating that linear drag dominates its motion.
See Section~\ref{sec:problems} Exercise~\ref{ex:reynolds} for more information
about the Reynold's number.
We model this
nonconservative force,
with the linear friction $\vec{F}_{drag} = -\eta \vec{v}_i$
sufficient so that
...
% [finish... discussion of Reynolds number]
This model should be familiar to readers
modeling Millikan's oil drops
...
%to
The equation of motion for an isolated particle 
\begin{equation}
  \eta \vec{v}_i = \vec{F}^l_{i} + \vec{F}^{D}(t).
    \label{eq:motion}
\end{equation}
where the viscosity coefficient $\eta = 1$.
...
[more background here - do the students necessarily know the Verlet method?  no]
...
and landscape period $\lambda = L/N_p = 1.825 a_0$.
...
When the superposition of $F^{AC}$ and $F^{DC}$ is large enough
...
A typical $v$ in experiment is on the order of $v \sim 1 \mu /s$,
thus $R \approx 10^-6$.
\label{ex:reynolds}
