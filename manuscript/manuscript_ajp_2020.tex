...
\begin{table}[h!]
\centering
\caption{Simulation units and comparable
...
period & $\lambda = 1.825 a_0$ & $\lambda = 3.5 \mu m$ \\
potential amplitude & $U_0 = 0.058 E_0$ & $U_0 = 75 k_B T \sim 2 J$ \\
temperature & $T_0 = 0$ \footnote{a. see Ex.~\ref{ex:brownian} to include temperature.} & $T \sim 300K $ \\
...
coincide with %the condition the is
...
\begin{enumerate}
\item {\bf Drag Models and Reynolds Numbers}
  ...
  $\vec{F}^{drag} = -b \vec{v} - c v^2 \hat{v} + ... $.
  ...
  but  $\rho \sim 10^3$ kg/cm$^3$ is a reasonable
...
\item{\bf Equation of Motion}
  ...
  The ratio of $m/\eta$ is %has units of time, % and is 
  ...
  Generally when solving ordinary differential equations,
  ...
  of motion to solve for the position of a particle
  $y_n$ at the $nth$ timestep.

  {\it invite/guide students to play with the timestep and study the results}
  %FOLLOW UP WITH THIS!
  ...
  [{\it explain Verlet and/or RK4...}]
  Demonstrate how the Verlet method
  simplifies to the Euler method when
  the particle acceleration $\vec{a}=0$.
 
  {\it invite/guide students to play with the timestep and study the results in comparison to Euler method}
...
\item {\bf Exploring Parameters of the Model}
  \label{ex:parameters}
...
\item {\bf Visualizing motion + phase plots}
  \label{ex:animation}
...
in the absence of collective effects of particle interactions.
...
%Lutz  created 1D circular channels by means of scanning optical tweezers in order to avoid the presence of lateral confinement walls
    ...
    \bibitem{Taylor2005} J. Taylor,  Classical mechanics. University Science Books (2005).
      ...
      \bibitem{Newman} M. Newman, Computational Physics... 
