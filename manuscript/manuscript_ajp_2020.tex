...
of an individual particle from location $\vec{r}_i$ %in the simulation
...
\caption{
  ...
  %is calculated from the gradient of the potential energy 
  %$\vec{F}^l = -\nabla U(\vec{r})$.
  ...
  A particle is %in the region between minima and maxima
...
%to explore the qualitative behavior
...
%Here we explore one case in the low frequency regime,
...
%JUNIPER2017
%The driving force is provided by a PI-542.2CD piezo-stage, controlled using the LabView interface, at driving 0.4 ≤ FAC/ζ ≤ 14 µm s−1, and a frequency of 1 velocities of 0.05 ≤ FDC/ζ ≤ 8 µm s−1. AC driving veloc
...
$\langle {v}_y \rangle = \lambda f$. % where $T = 1/f$.
...
%  On each of the first four modes,
%  we mark a values of $F^{dc}$ to illustrate in Fig.~\ref{fig:phase}.
...
%  \label{ex:phase}
  ...
  all cases exhibit some phase shifts %line thickening, 
  ...
  $F^{dc}$, % and frequency $f = 0.01$ and $F^{ac} = 0.2$.
  ...
  Fig.~\ref{fig:0}(a).
  %The maximum $\dot{\phi}$ %with a 1:1 mode locking,
  %occurs at $\phi \sim 0.4$,
  %and maximum $\phi$ occurs
  %when $\dot{\phi} \sim 0$.
  %
  %The dynamical properties are asymmetric,
  %with three nodes in the positive $\dot{\phi}$ values.
  %[insert analysis.]
  As $F^{dc}$ %the constant driving force 
  increases,
  nodes form in the phase diagram
  that occur due to repeated values
  of $\dot{\phi}$ over multiple phase positions.
  %The motion becomes quasiperiodic
  ...
  three nodes form as the particle moves across $3\lambda$.
    \begin{figure} %[h!]
      ...
      \caption{
        ...
        in Fig.~\ref{fig:1}. %, (e) $0.25$ and (f) $0.3$.
...
%\item
  ...
  over the period length $1000 \tau$.
    \begin{figure} %[h!]
      ...
      \caption{
        Motion of a particle with the same values of $F^{dc} = 0.07$ and $F^{ac} = 0.07$ as Fig.~\ref{fig:0} and decreasing frequency $f=$ (a) $0.05$, 
        (b) $0.022$, (c) $0.005$ and (d) $0.001$. %where  
  ...
  $n_b = 0$ with $n_f = 4$.
  In Fig.~\ref{fig:hops_back}(b) $F^{ac} = 0.4$
  $n_b = 2$ and $n_f = 6$.
...
moving through a viscous liquid at velocity $\vec{v}$ 
  ...
  %viscosity coefficient & $\eta = F_0 \tau / {a_0}^2 $ & $\eta \sim 10^{-3}$ Pa-s \\
  To first order the viscosity is $\eta \sim 10^{-3}$ Pa-s \cite{Volpe2013}.
  %or $9 \times 10^{-8} kg/s$ \cite{Juniper2017}.
  ...
  %Reproduce these results.  
    \begin{figure} %[h!]
      ...
      \caption{
        ...
        (b) $T/A_p = 3.5$ one hop occurs and %at $t = 130000$
...
\bibitem{Portugal2014} S. Portugal, T. Hubel, J. Fritz, S. Heese, D. Trobe, B. Voelkl, S. Hailes, A. M. Wilson and J. R. Usherwood.  Upwash exploitation and downwash avoidance by flap phasing in ibis formation flight. Nature {\bf 505}, 399 (2014).
  ...
  \bibitem{Singer1999} W. Singer. Striving for coherence. Nature, {\bf 397} 391, 1999.
    ...
    \bibitem{Benz1990}  S. P. Benz, M. S. Rzchowski, M. Tinkham, and C. J. Lobb, Fractional giant Shapiro steps and spatially correlated phase motion in 2D Josephson arrays, Phys. Rev. Lett. 64, 693 (1990); D. Dom{\'i}nguez and J. V. Jos{\'e}, Giant Shapiro steps with screening currents, Phys. Rev. Lett. 69,
514 (1992).
...
\bibitem{Reichhardt2000} C. Reichhardt, R. T. Scalettar, G. T., Zim{\'a}nyi, N. Gr{\o}nbech-Jensen,  Phase-locking of vortex lattices interacting with periodic pinning.  Phys. Rev. B {\bf 61}, R11914 (2000).
    ...
    \bibitem{Juniper2015} M. P. N. Juniper, A. V. Straube, R. Besseling, D. G. A. L. Aarts, and R. P. A. Dullens, Microscopic dynamics of synchronization in driven colloids. Nat. Commun. 6, 7187 (2015); 
      ...
      \bibitem{Juniper2017} Juniper, M. P. N., Zimmermann, U., Straube, A. V., Besseling, R., Aarts, D. G. A. L., L{\"o}wen, H., and Dullens, R. P. A.  Dynamic mode locking in a driven colloidal system: Experiments and theory. New Journal of Physics, 19(1). (2017).  %https://doi.org/10.1088/1367-2630/aa53cd
      
    \bibitem{Herrera-Velarde2008} S. Herrera-Velarde and R. Casta{\~n}eda-Priego, Superparamagnetic colloids confined in narrow corrugated substrates, Phys. Rev. E {\bf 77}, 041407 (2008).

    \bibitem{Herrera-Velarde2007} S. Herrera-Velarde and R. Casta{\~n}eda-Priego, J. Phys.: Condens. Matter 19, 226215 (2007).
...
steps in the force-velocity relationship.
 ...
 we can easily use 
  ...
  a non-driven colloid for $T/A_p = 3.0$, $3.5$, and $4.0$.
