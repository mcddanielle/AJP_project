...
%\item
  ...
  we show the position versus time of a simple particle
  ...
  of a non-driven colloid for $T/A_p = 0.5, 1.0$, and $2.0$.
...
%\item
  ...
  time period,
  for a total displacement of $n_f - n_b$ integer steps of $\lambda$
  In Fig.~\ref{fig:0},
  the particle moves forward through two minima, i.e. $n_f = 2$,
  and does not move backward a full minima - $n_b = 0$,
  in a single period, such that the pattern
  is characterized as $2(2,0)$.
  In experiments with single particles \cite{Juniper2015}, 
  hopping patterns of $n_f = 0 - 6$ for $n_b = 0$,
  $n_f = 0 - 5$ for $n_b = -1$,
  and
  $n_f = 2 - 3$ for $n_b = -2$
  were realized 
  ...
  In order to achieve
  backwards hops,
  a particle must have a high ratio of $F^{ac}/F^{dc}$.
  
  Additionally the frequency must be low so that
  the high force portions of the cycle last
  long enough to move a particle over substrate extrema.
