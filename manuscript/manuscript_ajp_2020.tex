...
by synchronizing its motion across the landscape %maxima 
to the periodic pattern of 
...
In Fig.~\ref{fig:0} 
a single driven particle adjusts
its motion so the particle traverses
...
where the landscape force is negative $F^l < 0$,
causing the particle to move into
...
and the motion of the particle
...
the particle is trapped in a single
minima so it oscillates back and forth with no net velocity.
...
\caption{(Average particle velocity in the y-direction $\langle v_{y} \rangle$ as a function of a constant driving force $F^{dc}$.}
\label{fig:1}
\end{figure}
\end{center}
