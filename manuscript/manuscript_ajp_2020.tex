...
temperature & $T_0 = 0$ \footnote{see Ex.~\ref{ex:brownian} to include temperature in the limit where $U_0 \sim k_B T$.} & $T \sim 300K $ \\
...
%\item
  ...
  phenomena.
  Particles change direction %randomly
  ...
  % (a) 
  In simulation we define our temperature parameter
  $k_b T/E_0 \rightarrow T$
  since we set the constants to unity.
  This allows us to choose
e  a temperature that is easily compared to
  the other system parameters that scale the forces.
  For instance, 
  with
  ...
  when Brownian motion is included
  in the model.
  Confirm that a particle will hop a barrier
