...
control the rate and length of the particle motions 
...
for $\Delta t = 10^5$ simulation timesteps
and 
sample the average velocity $\langle v_y \rangle $ 
as a function of $F^{dc}$.
We plot the results in Fig.~\ref{fig:1}
for $F^{ac} = 0.05$, $f=0.01$, and the landscape force $\vec{F}^{l}$
shown in Fig.~\ref{fig:0}.
The steps in $\langle v_y \rangle$ occur because 
...
At low $F^{dc}$, the particle starts and ends in the same minima.
...
\caption{Average particle velocity  $\langle v_{y} \rangle$ as a function of $F^{dc}$.
  We fix the remaining parameters in $F^d(t)$ as $F^{ac}=0.05$ and $f = 0.01$ as in Fig.~\ref{fig:0}}.
\label{fig:1}
\end{figure}
\end{center}
