...
and can be visualized with %demonstrated
...
using pacemaker (an electrical generator)
pulses to regulate a heart beat
or using a pulsed light to control the pattern of flashing
fireflies \cite{Agrawal2013}.
These dynamical drive studies can be performed
on coupled oscillators %\cite{Kuramoto193?}
...
much like a skateboarder in a half-pipe.  % \cite{}.
%Single particle models avoid many-body effects
%while offering insight.
The applied driving force can be constant (dc) or sinusoidal (ac).
With a pure 
dc drive the landscape modulates 
the average particle velocity.
An ac drive creates mode-locking,
...
of mode-locking,
which cannot be easily imaged in electronics.
%experimental - colloids specifically
Colloidal particles trapped in light fields are %have proven
a useful medium for studying complex dynamical behaviors.  %cite
Colloid particles are micron-sized,
a relatively large size to control and image.
The interaction forces between colloids can be modified
by tuning the chemistry of the suspending fluid or surface ligands
of the colloidal particles.
Because colloids are easy to control and study, 
they are used as experimental models
for systems relatively hard to access and visualize,
such as cold atoms or electron gases \cite{Grier2003}. %ELABORATE AND CITE
%Recent work with atomic systems such as Bose-Einstein condensates \cite{Zhang2020}
Mode-locking 
in colloids has been acheived in systems interacting via magnetic dipoles 
or electric forces 
in experiment \cite{Juniper2015, Lutz2004} and
simulation \cite{Herrera-Velarde2007, Herrera-Velarde2008}.

%-----------------------------------------------------------
%particle interaction types
%Particles which interact over long distances
%include colloids, magnetic beads, superconducting vortices, dusty plasmas, electron gases. [mor%e detail and references]
...
%PHASE DIAGRAMS SHOWNING COUPLING, OR QUASIPERIODIC COUPLING
...
\end{equation}
%v is the standard I note for substrates
...
\item Frenkel-Kontorova and competing length scales for low temperature systems where thermal fluctuations are neglible
...
514 (1992).
    ...
    \bibitem{Herrera-Velarde2008} S. Herrera-Velarde and R. Castañeda-Priego, Superparamagnetic colloids confined in narrow corrugated substrates, PHYSICAL REVIEW E {\bf 77}, 041407 (2008).
...
%      Lutz et al. 关11,12兴 created 1D circular channels by means of scanning optical tweezers in order to avoid the presence of lateral confinement walls
    ...
    \bibitem{Tarucha1995}  Tarucha, S.; Honda, T.; Saku, T. Solid State Commun. 1995, 94, 413.
    ...
    \bibitem{Allen2020} Allen and Tildsley, The Computer Simulation of Liquids. 
