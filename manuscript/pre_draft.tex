...
\begin{abstract}
  ...
  mimick experimental studies of driven colloidal particles
  confined by light-fields.
  Much like experiments on colloids, 
  we control the particle interactions,
  external force and confining environment.
  We assume the particles move through
  a viscous liquid,
...
Synchronization is a universal phenomena \cite{Pikovsky2003}.
%Christiaan Huygens 
%[has been adapted into many useful technologies...]
Huygens 1665 observation of
in-phase motions of pendulum clocks,
provides a classic example,
where two periodic oscillators
were synchronized due to
a coupling by the mounting wall \cite{Bennet2002}.
Such physical couplings are also observed
in [examples]

Biological systems benefit from cooperative
synchronzation.
Birds coordinate flapping of wings in order to [] \cite{},
to simultaneous firing of neurons in cardiac muscle \cite{}
and brain tissue \cite{}.

Such systems can be driven to form new synchronized patterns,
for instance
firefly [swarms] signal in time with a pulsed light emitting diode \cite{},
[more examples]
Driven coupled oscillations are sufficiently
broad and useful to be the subject of computational modeling.


%phase locking

Synchronization can be modeled with numerical studies of confined 
driven particles.
...
\bibitem{Bennett2002} M. Bennett, M.F. Schatz, H. Rockwood, and K. Wiesenfeld, Huygen's clocks, Proc. Roy. Soc. A 458, 563 (2002).
