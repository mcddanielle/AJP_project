...
A single pendulum 
can swing in time with an %be driven or damped by
driving force
...
its trajectory.
The particle is confined in a one-dimensional %DM reduced to 1D
simulation of length $L=46.\bar{6}\,a_0$, %DM 1D
where $a_0$ is a dimensionless unit of length.
(In the simulations we set $a_0 = 1$.)
The particle 
has
position $\vec{r} = y \hat{y}$ %DM 1D
...
by the transformation $y+L \rightarrow y$. %DM 1D
...
The confining force on the particle %$i$ DM removed
 is given by
 \begin{equation}
...
%superconductors with periodic pinning arrays, and 
...
\begin{problem}{Write your own MD code}
  ...
  $\vec{v} = d\vec{r}/dt$ 
  with the %DM sub ``with'' for using
  Euler method.
  %DM - fix Equation/Eq. discontinuity
...
we recommend a   
  ...
  \item[(a)] {\bf Initialization.}
    ...
    to subroutines.
    %DM - more clearly defined task in active / command tense
